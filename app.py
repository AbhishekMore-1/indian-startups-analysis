...
def app():
    ...
    def data_work():
        ...
        data.columns=['Date', 'StartupName', 'IndustryVertical', 'SubVertical',
    ...
    """)
    st.sidebar.subheader("Developed By Team Four Bits")
    st.sidebar.markdown("""
    - Abhishek More
    - Aditya Birangal
    - Shubham Kotgire
    ...
    top_filter = st.slider('Top N Funded Cities', 3, 10, 5)  # min: 3, max: 10, default: 10
    label=np.arange(0,top_filter)
    cities=data.groupby(by=['CityLocation'])['AmountInUSD'].sum().sort_values(ascending=False).head(top_filter).reset_index()
    cities.columns=["City","AmountInUSD"]
    fig=go.Figure(data=[go.Bar(y=cities.AmountInUSD,x=cities.City, marker={'color':label})])
    fig.data[0].marker.line.width = 3
    fig.data[0].marker.line.color = "black" 
    fig.update_layout(autosize=True,plot_bgcolor='rgb(275, 275, 275)')
    st.plotly_chart(fig)

    st.subheader('Year wise Total Fund!')
    year = data[['Date','AmountInUSD']]
    year['Date'] = pd.to_datetime(year['Date']).dt.year
    year = year.groupby(by=['Date'])['AmountInUSD'].sum().reset_index()
    year.columns=["Year","AmountInUSD"]
    fig=go.Figure(data=[go.Line(y=year.AmountInUSD,x=year.Year, marker={'color':label})])
    fig.data[0].marker.line.width = 3
    fig.data[0].marker.line.color = "black" 
    fig.update_layout(autosize=True,plot_bgcolor='rgb(275, 275, 275)')
    st.plotly_chart(fig)
    ...
    ***
    """)
    st.subheader("""
    Developed with ❤️ by Team Four Bits
    """) 
